...
@section('titulo', 'Certificado de calidad '.$inspeccion->code)
...
@section('contenido')
    ...
    @if ($inspeccion->estado === \App\Models\Inspection::RECHAZADO)
        ...
        <div class="mx-auto max-w-3xl px-3 pt-4">
            <div class="rounded-lg border-l-4 border-rojo-600 bg-rojo-50 p-4">
                <p class="text-sm font-bold text-rojo-900">PRODUCTO NO CONFORME</p>
                <p class="mt-1 text-sm text-rojo-800">
                    Esta inspección resultó no conforme. Comunicate con el área de Control de
                    Calidad de Plásticos Carmen antes de utilizar el material.
                </p>
            </div>
        </div>
    @endif

    <main class="mx-auto max-w-3xl px-3 py-5 print:m-0 print:max-w-none print:p-0">
        @include('inspecciones._boleta', ['publico' => true])
    </main>

    <footer class="no-imprimir mx-auto max-w-3xl px-4 pb-8 text-center text-xs text-slate-400">
        <p>Plásticos Carmen S.R.L. &middot; Control de Calidad</p>
        <p class="mt-1">
            Este certificado corresponde únicamente al lote y proceso identificados arriba.
        </p>
    </footer>
@endsection
