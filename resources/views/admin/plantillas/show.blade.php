    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            <form method="POST" action="{{ route('admin.plantillas.duplicar', $plantilla) }}">
                @csrf
                <button type="submit" class="btn-secundario">Crear revision nueva</button>
            </form>
            @unless ($plantilla->active)
...
@section('contenido')
    ...
    @if ($usos > 0)
        <div class="mb-5 rounded-lg bg-amber-50 p-4 ring-1 ring-amber-200">
            <p class="text-sm font-semibold text-amber-900">Esta plantilla ya tiene boletas emitidas</p>
            <p class="mt-1 text-sm text-amber-800">
                Cambiar una especificacion aca alteraria como se muestran boletas ya emitidas.
                Si necesitas cambiar tolerancias, usa <strong>Crear revision nueva</strong>: las
                boletas viejas quedan atadas a esta revision y las nuevas usan la siguiente.
            </p>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.plantillas.update', $plantilla) }}" class="tarjeta mb-5 p-4 sm:p-5">
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label for="muestras_max" class="etiqueta">Max. *</label>
    ...
    <div class="tarjeta mb-5 overflow-hidden">
        <div class="border-b border-slate-200 px-4 py-3 sm:px-5">
            ...
            <p class="mt-0.5 text-xs text-slate-500">Toca un parametro para editarlo.</p>
        </div>

        @if ($plantilla->parameters->isEmpty())
            <p class="px-4 py-8 text-center text-sm text-slate-500 sm:px-5">
                Todavia no hay parametros. Agregá el primero con el formulario de abajo.
            </p>
        @else
                        ...
                        <button type="button" @click="abierto = !abierto"
                            ...
                            <span class="min-w-0">
                                <span class="block text-sm font-medium text-slate-800">
                                    ...
                                    {{ $p->etiqueta }}
                                    @if ($p->requerido)
                                        <span class="badge ml-1 bg-red-50 text-red-700 ring-red-200">critico</span>
                                    @endif
                                </span>
                        ...
                        <div x-show="abierto" x-cloak class="border-t border-slate-100 bg-slate-50/50 px-4 py-4 sm:px-5">
                            <form method="POST" action="{{ route('admin.parametros.update', $p) }}">
                                ...
                                <div class="mt-4 flex flex-wrap gap-2">
                                    <button type="submit" class="btn-primario">Guardar parametro</button>
                            ...
                            <form method="POST" action="{{ route('admin.parametros.destroy', $p) }}" class="mt-3"
                                  onsubmit="return confirm('Eliminar el parametro {{ $p->label }}?')">
                                @csrf
                                @method('DELETE')
    ...
    <div x-data="{ abierto: {{ $plantilla->parameters->isEmpty() ? 'true' : 'false' }} }" class="tarjeta p-4 sm:p-5">
        <button type="button" @click="abierto = !abierto" class="flex w-full items-center justify-between text-left">
            <span>
                <span class="block text-sm font-semibold text-slate-900">Agregar parametro de ensayo</span>
                <span class="block text-xs text-slate-500">Una caracteristica nueva a medir en este proceso.</span>
            </span>
        ...
        <div x-show="abierto" x-cloak class="mt-4 border-t border-slate-200 pt-4">
            <form method="POST" action="{{ route('admin.parametros.store', $plantilla) }}">
                ...
                <div class="mt-4">
                    <button type="submit" class="btn-primario">Agregar parametro</button>
                </div>
            </form>
        </div>
    </div>
